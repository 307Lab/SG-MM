...
class PointToPointPlanner(EEPlannerFullPython):
    ...
    if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
        HEIGHT_INFLATION = 0.05
        PLANNER_RESOLUTION = 0.01
    ...
    def generate_base_feasible_eeweights(self,
        ...
        ee_weights = (self.map_to_weights(self._map,
                                          ...
                                          height_preference= True)  #self.eval)
                      + inversed_map)
        return ee_weights
    ...
    def calc_xyplan(map: "Map", start, goal, ignore_below_height: float, weights: Optional[np.ndarray] = None):
        ...
        if calc_euclidean_tf_dist(start, goal) < SQROOT_TWO * map.global_map_resolution:
            ...
            if (rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator" ) and (np.max(astar_start) > 49 or np.max(astar_goal) > 49):
                print("np.max(astar_start):", np.max(astar_start))
                print("np.max(astar_goal):", np.max(astar_goal))
                astar_start = np.clip(astar_start, 0, 49)
                astar_goal = np.clip(astar_goal, 0, 49)
