...
class Map:
    def __init__(self,
        ...
        self._floorplan_img_orig_with_door = None
    ...
    def get_empty_floorplan(size_meters: Tuple[float, float], resolution: float):
        ...
        # wall around the map
        if rospy.get_param("robot_name") != "fg_125_manipulator" and rospy.get_param("robot_name") != "slide_rail_manipulator":
            map[[0, int(size_meters[1] / resolution / 2),-1], :] = True
    ...
    def load_map(map: Union[str, Path, Tuple[float, float]], current_resolution: float, target_resolution: float, max_map_height, pad: bool = True) -> Image:
        ...
        if pad:
            ...
            if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
                padding = round(0.05 / target_resolution) # 设置5cm的四周墙
    ...
    def meter_to_pixels(self, xypose_meter: np.ndarray, round: bool = True, origin_W_meter: float = None, origin_H_meter: float = None, resolution: float = None):
        ...
        if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
            xypose_pixel = np.clip(xypose_pixel, 0, 99)
        return xypose_pixel
    ...
    def _get_local_map_gt(self, current_location_tf: Transform, plan_meter: Optional[list] = None, ignore_below_height: float = 0.0) -> np.ndarray:
        ...
        tf = cv2.getRotationMatrix2D(center=tuple(center_pixel_cropped_2x + translation), angle=np.rad2deg(-yaw), scale=1)
        tf[:, 2] -= translation
    ...
    def draw_startpose_and_goal(self, robotenv, joint_distribution: Union[str, list], gripper_goal_tip_fn: Union[Callable, list]):
        ...
        while True:
            ...
            if not real_world and not isinstance(self, DummyMap):
                ...
                if rospy.get_param("robot_name") == "fg_125_manipulator" and rospy.get_param("robot_name") == "slide_rail_manipulator":
                    nr_collisions = ee_cost.sum() / PointToPointPlanner.OCCUPIED_COST
                else:
                    # 忽略最后50厘米的碰撞，以便让夹持器的目标位于物体上方
    ...
    def map_reset(self):
        ...
        if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
            ...
            self._floorplan_img = self._floorplan_img_orig_with_door.copy()
            self._floorplan_img_filled = self._floorplan_img_orig_with_door.copy()
        self.inflated_map = dict()
    ...
    def __init__(self, world_type, global_map_resolution, local_map_resolution, robot_frame_id, inflation_radius):
        if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
            super(DummyMap, self).__init__(world_type=world_type, global_map_resolution=global_map_resolution, local_map_resolution=local_map_resolution,
                                       robot_frame_id=robot_frame_id, inflation_radius=inflation_radius, floorplan=(0.4, 0.6),
                                       update_from_global_costmap=False, max_map_height=0.3, local_map_size_meters=0.3, pad_floorplan_img=False)
        else:
            super(DummyMap, self).__init__(world_type=world_type, global_map_resolution=global_map_resolution, local_map_resolution=local_map_resolution,
                                        robot_frame_id=robot_frame_id, inflation_radius=inflation_radius, floorplan=(12, 12),
...
class SceneMap(Map):
    def __init__(self,
        ...
        if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
            super(SceneMap, self).__init__(world_type=world_type, global_map_resolution=global_map_resolution, local_map_resolution=local_map_resolution,
                                       robot_frame_id=robot_frame_id, inflation_radius=inflation_radius, floorplan=(0.4, 0.4),
                                       update_from_global_costmap=False, max_map_height=0.3, local_map_size_meters=0.3,
                                       initial_base_rng_x=initial_base_rng_x, initial_base_rng_y=initial_base_rng_y, initial_base_rng_yaw=initial_base_rng_yaw,
...
class SimpleObstacleMap(SceneMap):
    def __init__(self,
        ...
        self._obstacle_spacing = 3.0  #obstacle_spacing
        self.offset_std = 0.8  #offset_std
        if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
            self._obstacle_spacing = 0.15
            self._obstacle_spacing = 0.0375
    ...
    def _draw_initial_base_pose(self):
        ...
        if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
            x = self.np_random.uniform(-0.38, 0)
            y = self.np_random.uniform(-0.38, 0)
            yaw = 0
    ...
    def get_varying_scene_objects(self) -> List[SpawnObject]:
        ...
        for xi, x in enumerate(np.arange(-self._map_W_meter / 2, self._map_W_meter / 2, self._obstacle_spacing)):
            ...
            for y in np.arange(-self._map_H_meter / 2, self._map_H_meter / 2, self._obstacle_spacing):
                ...
                # xx = self.np_random.uniform(0.025, 1.0)
                # yy = self.np_random.uniform(0.025, 0.5)
                # zz = self.np_random.uniform(0.2, 0.8) if self.np_random.random() > 0.33 else self.max_map_height
