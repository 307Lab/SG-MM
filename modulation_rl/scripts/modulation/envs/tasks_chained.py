...
class ObstacleConfigMap(SceneMap):
    def __init__(self,
                 ...
                 initial_base_rng_x=(-4.0, -3.0),
                 initial_base_rng_y=(4.0, 3.0),
    ...
    def _add_rnd_obstacles(self) -> List[SpawnObject]:
        ...
        obj0 = SpawnObject(f"obstacle0", WorldObjects.cylinder_inpath,
                           lambda: Pose(Point(self.np_random.uniform(-2.0, -1.0), self.np_random.uniform(2.0, 4.0), 0.24), Quaternion(0, 0, 0, 1)))
        # between robot and door-kallax.
        obj1 = SpawnObject(f"obstacle1", WorldObjects.cylinder_inpath,
                           lambda: Pose(Point(self.np_random.uniform(1.0, 2.0), self.np_random.uniform(2.0, 4.0), 0.24), Quaternion(0, 0, 0, 1)))
        # between robot and drawer-kallax
        obj2 = SpawnObject(f"obstacle2", WorldObjects.cylinder_inpath,
                           lambda: Pose(Point(self.np_random.uniform(-4.0, -1.0), self.np_random.uniform(-3.0, -2.0), 0.24), Quaternion(0, 0, 0, 1)))
        
        obj3 = SpawnObject(f"obstacle2", WorldObjects.cylinder_inpath,
                           lambda: Pose(Point(self.np_random.uniform(1.0, 4.0), self.np_random.uniform(-3.0, -2.0), 0.24), Quaternion(0, 0, 0, 1)))
...
class PickNPlaceChainedTask(BaseChainedTask):
    ...
    def get_goal_objects(self, start_table_pos=(5.0, 3.0, 5.0, 2.0), end_table_rng=(-5.0, 5.0, -5.0, -4.5)):
    # def get_goal_objects(self, start_table_pos=Point(x=3.3, y=0, z=0), end_table_rng=(-1.5, 2, -3, -1.5)):
        objects = []
        # pick table
        endx = self.np_random.uniform(start_table_pos[0], start_table_pos[1])
        endy = self.np_random.uniform(start_table_pos[2], start_table_pos[3])
        start_table_pose = Pose(Point(x=endx, y=endy, z=0), Quaternion(0, 0, 1, 0))
        objects.append(SpawnObject("pick_table", self._pick_table, start_table_pose))

        # shelf_pos = [self.map.door_pos[0], self.map.door_pos[1], 0.24]
    ...
    def draw_goal(self) -> List[TaskGoal]:
        ...
        image_array = np.array(self.map._floorplan_img_orig.copy())
        rnd =  np.random.randint(80, image_array.shape[0] - 80)
        image_array[int(image_array.shape[0] / 2), rnd - 40 : rnd + 40] = 0
        self.map._floorplan_img_orig_with_door = Image.fromarray(image_array)
        door_pos = [(- int(image_array.shape[0] / 2) + rnd) * self.local_map_resolution, 0.0]

        target_shelf_pose = Pose(Point(x=door_pos[0], y=door_pos[1], z=0.65), Quaternion(0, 0, 0, 1))
        obj_origin_input, release_input = self.get_kallax_door_gmm_input(target_shelf_pose)
        ...
        return [
                TaskGoal(gripper_goal_tip=in_front_of_obj_loc, end_action=GripperActions.NONE,
