...
def sample_circle_goal(base_tf: list, goal_dist_rng: Tuple[float, float], goal_height_rng: Tuple[float, float], map_width: float, map_height: float, np_random, distribution: str = "uniform") -> list:
    ...
    if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
        half_w = 0.25
        half_h = 0.25
        yaw = np_random.uniform(0.0, 2 * np.pi)  # 偏航角：任意
        pitch = np_random.uniform(-np.radians(20), np.radians(90))  # 俯仰角：-90° ~ +20°
        roll = 0.0  # 翻滚角固定为 0
        RPY_goal = [roll, pitch, yaw]
        
        # 以map为中心采样目标点, 其他机器人以底座base为中心采样目标点
        return [min(max(x, -half_w), half_w),
            min(max(y, -half_h), half_h),
            z] + RPY_goal
    else:
