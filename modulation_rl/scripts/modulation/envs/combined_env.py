...
class CombinedEnv(Env):
    ...
    def ik_reward(self, info: dict) -> float:
        ...
        if self._robot.ikslack_sol_dist_reward == "ik_fail":
            ...
            # l2-norm as penalty
            ik_reward = scaled_failed_reward
            # self.manipulability_max = max(self.manipulability_max, info["manipulability_w"])
            # ik_reward += (info["manipulability_w"] / self.manipulability_max) * 0.05
            # print("manipulability_w: ", self.manipulability_max)
            # self.manipulability_max = max(self.manipulability_max, info["manipulability_w"])
            # safe_denominator = max(self.manipulability_max, 1e-6)
            # safe_manip = np.nan_to_num(info["manipulability_w"] / safe_denominator, nan=0.0, posinf=1.0, neginf=0.0)
            # ik_reward += safe_manip * 0.05
            
            self.manipulability_max = max(self.manipulability_max, info["manipulability_w"])
            info["manipulability_reward"] = ik_reward + (info["manipulability_w"] / self.manipulability_max)
