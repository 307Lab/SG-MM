...
class RobotEnv(Env):
    def __init__(self,
        ...
        self.robot_prefix = ""
        if node_handle_name[:5] == "robot":
            self.robot_prefix = node_handle_name[:6]  
        ...
        if env == 'pr2':
            ...
            self.robot_config['ik_joint_model_group_name'] = 'manipulator' if learn_torso else self.robot_config['joint_model_group_name']
            robot_fn = RobotHusky
            self.drive = "diff"
        elif env == 'fg_125_manipulator':
            self.robot_config['ik_joint_model_group_name'] = 'manipulator' if learn_torso else self.robot_config['joint_model_group_name']
            robot_fn = RobotFg125
            self.drive = "slide"
        elif env == 'slide_rail_manipulator':
            self.robot_config['ik_joint_model_group_name'] = 'manipulator' if learn_torso else self.robot_config['joint_model_group_name']
            robot_fn = RobotSO101
            self.drive = "slide"
        elif env == 'tiago':
        ...
        self._robot_cpp = robot_fn(seed,
                                   self.robot_prefix, #如果是要多个环境并行，这里传进去的是命名空间前缀, 而不是node_handle_name
                                   init_controllers,
                                   world_type,
    ...
    def reset(self,
        ...
        if rospy.get_param("robot_name") == "fg_125_manipulator" or rospy.get_param("robot_name") == "slide_rail_manipulator":
            initial_joint_values['rail_joint_1'] = initial_base_pose[0]
            initial_joint_values['rail_joint_2'] = initial_base_pose[1]
        else:
    ...
    def step(self, base_actions, joint_value_deltas, next_desired_gripper_tf, execute_cmds: bool = True) -> Tuple[RobotObs, float, dict]:
        ...
        self.trajectory.add(robot_obs)
        manipulability_w = self._robot_cpp.get_manipulability()
        ...
        robot_info = {}
        robot_info['manipulability_w'] = manipulability_w
