...
def get_parser():
    ...
    parser.add_argument('--total_steps', type=int, default=10_000_000, help='Total number of action/observation steps to take over all episodes')
    ...
    # Env
    #################################################
    parser.add_argument('--env', type=str.lower, default='pr2', choices=['pr2', 'tiago', 'hsr', 'husky_ur5', 'fg_125_manipulator', 'slide_rail_manipulator'], help='')
    ...
    parser.add_argument('--robot_prefix', type=str, default=None, help='')
    return parser
...
def launch_ros(main_path: Path, config, task: str, no_gui: bool = False, always_relaunch: bool = None, algo="rl", pure_analytical=None):
    ...
    out = subprocess.run(["python", script] + args, env=my_env)
