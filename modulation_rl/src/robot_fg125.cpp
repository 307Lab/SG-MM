...
#include <modulation_rl/robot_fg125.h>

using namespace std;
namespace py = pybind11;

RobotFg125::RobotFg125(uint32_t seed,
...
void RobotFg125::setup() {
    if (init_controllers_) {
        arm_client_.reset(new TrajClientFg125("/eff_joint_traj_controller/follow_joint_trajectory"));
        ...
        gripper_client_.reset(new TrajClientFg125("/gripper_controller/follow_joint_trajectory"));
...
tf::Transform RobotFg125::calcDesiredBaseTf(const tf::Transform &base_tf,
...
void RobotFg125::calcDesiredBaseCommand(const tf::Transform &current_base_tf,
...
void RobotFg125::sendArmCommands(const vector<vector<double>> &joint_values, vector<double> &timestamps) {
    vector<string> joint_names = joint_model_group_->getVariableNames();
...
void RobotFg125::openGripper(double position, bool wait_for_result) {
...
void RobotFg125::closeGripper(double position, bool wait_for_result) {
