...
#include <modulation_rl/robot_slide_rail_manipulator.h>

using namespace std;
namespace py = pybind11;

RobotSO101::RobotSO101(uint32_t seed,
...
void RobotSO101::setup() {
    if (init_controllers_) {
        arm_client_.reset(new TrajClientSO101("/manipulator/follow_joint_trajectory"));
        while (!arm_client_->waitForServer(ros::Duration(5.0))) {
            py::print("Waiting for the /manipulator/follow_joint_trajectory action server to come up");
        }

        // torso_client_.reset(new TrajClientHusky("/torso_controller/follow_joint_trajectory"));
        ...
        gripper_client_.reset(new TrajClientSO101("/gripper/follow_joint_trajectory"));
        while (!gripper_client_->waitForServer(ros::Duration(5.0))) {
            py::print("Waiting for the /gripper/follow_joint_trajectory action server to come up");
        }
...
tf::Transform RobotSO101::calcDesiredBaseTf(const tf::Transform &base_tf,
...
void RobotSO101::calcDesiredBaseCommand(const tf::Transform &current_base_tf,
...
void RobotSO101::sendArmCommands(const vector<vector<double>> &joint_values, vector<double> &timestamps) {
    vector<string> joint_names = joint_model_group_->getVariableNames();
    ...
    for (int k = 0; k < joint_values.size(); k++) {
        arm_goal_.trajectory.points[k].positions.resize(joint_names.size());
        arm_goal_.trajectory.joint_names.resize(joint_names.size());
...
void RobotSO101::openGripper(double position, bool wait_for_result) {
    control_msgs::FollowJointTrajectoryGoal goal;

    // The joint names, which apply to all waypoints
    goal.trajectory.joint_names.push_back("gripper_finger1_finger_joint");
    goal.trajectory.joint_names.push_back("gripper_finger2_finger_joint");
...
void RobotSO101::closeGripper(double position, bool wait_for_result) {
