...
#include <modulation_rl/robot_husky.h>

using namespace std;
namespace py = pybind11;

RobotHusky::RobotHusky(uint32_t seed,
...
void RobotHusky::setup() {
    if (init_controllers_) {
        arm_client_.reset(new TrajClientHusky("/eff_joint_traj_controller/follow_joint_trajectory"));
        while (!arm_client_->waitForServer(ros::Duration(5.0))) {
            py::print("Waiting for the /eff_joint_traj_controller/follow_joint_trajectory action server to come up");
        }
        ...
        gripper_client_.reset(new TrajClientHusky("/gripper_controller/follow_joint_trajectory"));
...
tf::Transform RobotHusky::calcDesiredBaseTf(const tf::Transform &base_tf,
...
void RobotHusky::calcDesiredBaseCommand(const tf::Transform &current_base_tf,
...
void RobotHusky::sendArmCommands(const vector<vector<double>> &joint_values, vector<double> &timestamps) {
    vector<string> joint_names = joint_model_group_->getVariableNames();
...
void RobotHusky::openGripper(double position, bool wait_for_result) {
...
void RobotHusky::closeGripper(double position, bool wait_for_result) {
